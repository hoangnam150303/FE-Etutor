...
import { MenuOutlined, PhoneOutlined, SendOutlined, HeartOutlined, BellOutlined, SearchOutlined, AudioOutlined, CameraOutlined, PaperClipOutlined, SmileOutlined } from "@ant-design/icons";
...
const UserChat = () => {
    ...
    const filteredChats = chatList.filter(chat => chat.name.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    return (
        <div className="flex flex-col h-screen">
            ...
            <div className="flex flex-1 h-full rounded-md relative">
                ...
                <div className={`fixed inset-y-0 left-0 z-50 bg-white w-3/4 max-w-xs p-4 transition-transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:relative lg:w-1/5 border-r border-gray-200`}>
                    <button className="lg:hidden absolute top-4 right-4" onClick={() => setIsSidebarOpen(false)}>✖</button>
                    ...
                    <div className="space-y-4">
                        {filteredChats.map((chat) => (
                            <div
                                key={chat.id}
                                className={`flex items-center space-x-3 cursor-pointer p-2 rounded ${selectedChat?.id === chat.id ? "bg-gray-200" : "hover:bg-gray-100"}`}
                                onClick={() => { setSelectedChat(chat); setIsSidebarOpen(false); }}
                            >
                                <Avatar src="https://placehold.co/40x40" />
                                <div>
                                    <div className="font-semibold">{chat.name}</div>
                                    <div className="text-sm text-gray-500">{chat.messages[0]}</div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex-1 flex flex-col">
                    {selectedChat ? (
                        <>
                            ...
                            <div className="flex justify-between p-4 border-b border-gray-300">
                                <div className="flex items-center">
                                    <button className="lg:hidden mr-3" onClick={() => setIsSidebarOpen(true)}>
                                        <MenuOutlined />
                                    </button>
                                    <Avatar src="https://placehold.co/40x40" />
                                    <div className="ml-3">
                            ...
                            <div className="flex-1 p-4 overflow-y-auto">
                                {selectedChat.messages.map((msg, index) => (
                                    <div key={index} className="flex mb-4">
                                        <Avatar src="https://placehold.co/40x40" />
                                        <div className="ml-3 p-3 bg-gray-200 rounded-lg max-w-xs">{msg}</div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="p-4 border-t border-gray-300 flex items-center">
                                ...
                                <input
                                    ...
                                    style={{ display: "none" }}
                                />
                                <PaperClipOutlined className="mr-3 text-gray-500" onClick={handleFileClick} />
                                {file && <span className="text-gray-600 text-sm mr-3">{truncateFileName(file.name, 10)}</span>}
                        ...
                        <div className="flex flex-1 justify-center items-center text-gray-500">
                            ...
                            <span className="hidden lg:block">Chọn một cuộc trò chuyện để bắt đầu</span>
                        </div>
                    )}
                </div>
            </div>
